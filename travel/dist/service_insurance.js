 ...
 */
import { call_guard, call_machine, call_permission, call_repository, call_service, WOWOK } from 'wowok_agent';
import { sleep, TESTOR, TEST_ADDR, result, PAY_TYPE, PUBKEY, INSURANCE_PRODUCT } from './common';
var BUSINESS;
(function (BUSINESS) {
    BUSINESS[BUSINESS["adjuster"] = 1000] = "adjuster";
    BUSINESS[BUSINESS["finance"] = 1001] = "finance";
})(BUSINESS || (BUSINESS = {}));
;
export var INSURANCE_MACHINE_NODE;
(function (INSURANCE_MACHINE_NODE) {
    INSURANCE_MACHINE_NODE["Report_Incident"] = "Report Incident";
    INSURANCE_MACHINE_NODE["Emergency_Treatment"] = "Emergency Treatment";
    INSURANCE_MACHINE_NODE["Amount_claim"] = "Amount Claim";
    INSURANCE_MACHINE_NODE["Insurance_Payment"] = "Insurance Payment";
})(INSURANCE_MACHINE_NODE || (INSURANCE_MACHINE_NODE = {}));
...
const service = async (machine_id, permission_id, repository_id) => {
...
const machine_guards_and_publish = async (machine_id, permission_id, repository_id) => {
...
const service_guards_and_publish = async (machine_id, permission_id, service_id) => {
    ...
    const guard_id = await result('Guard', await call_guard({ data: data1 }));
    if (!guard_id)
        WOWOK.ERROR(WOWOK.Errors.Fail, 'guard_service_withdraw');
...
const guard_accident_recorded = async (machine_id, permission_id, repository_id) => {
    const data = { namedNew: {},
        description: 'Record incident report',
        table: [{ identifier: 1, bWitness: true, value_type: WOWOK.ValueType.TYPE_ADDRESS }, // progress witness
        ],
        root: { logic: WOWOK.OperatorType.TYPE_LOGIC_AND, parameters: [
                { query: 105, object: repository_id, parameters: [
    ...
    if (!guard_id)
        WOWOK.ERROR(WOWOK.Errors.Fail, 'guard_accident_recorded');
...
const guard_amount_claim = async (machine_id, permission_id, repository_id) => {
    const data = { namedNew: {},
        description: 'Record incident report',
        table: [{ identifier: 1, bWitness: true, value_type: WOWOK.ValueType.TYPE_ADDRESS }, // progress witness
        ],
    ...
    const guard_id = await result('Guard', await call_guard({ data: data }));
    if (!guard_id)
        WOWOK.ERROR(WOWOK.Errors.Fail, 'guard_amount_claim');
    const data2 = { object: machine_id,
...
const guard_insurance_payment = async (machine_id, permission_id, repository_id) => {
    const data = { namedNew: {},
        ...
        root: { logic: WOWOK.OperatorType.TYPE_LOGIC_AND, parameters: [
                ...
                { logic: WOWOK.OperatorType.TYPE_LOGIC_AS_U256_GREATER_EQUAL, parameters: [
                        ...
                        { query: 112, object: repository_id, parameters: [
                                { identifier: 1 },
                                { value: 'amount', value_type: WOWOK.ValueType.TYPE_STRING }
                            ] }
                    ] },
    ...
    const guard_id = await result('Guard', await call_guard({ data: data }));
    if (!guard_id)
        WOWOK.ERROR(WOWOK.Errors.Fail, 'guard_insurance_payment');
    const data2 = { object: machine_id,
...
const permission = async () => {
    ...
    const data = { description: 'Outdoor accident insurance', object: { name: 'insurance permission' },
...
const machine = async (permission_id) => {
    const data = { description: 'machine for an insurance', object: { name: 'machine', permission: permission_id },
