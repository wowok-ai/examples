import { GuardMaker, Guard, MODULES, OperatorType, Passport, ValueType, ContextType, GuardParser } from 'wowok';
export const test_guard_launch_creator_equal = async (protocol, param) => {
...
export const test_guard_launch_everyone = async (protocol, param) => {
    Guard.everyone_guard(protocol.sessionCurrent());
};
export const test_guard_launch_substring = async (protocol, param) => {
    let maker = new GuardMaker();
...
export const test_guard_future_object = async (protocol, param) => {
...
export const test_guard_to_object = async (protocol, param) => {
...
export const passport_query = async (param) => {
    let g1 = param.get('guard::Guard') ? param.get('guard::Guard')[0] : undefined;
    let g2 = param.get('guard::Guard') ? param.get('guard::Guard')[1] : undefined;
...
export const test_guard_passport = async (protocol, param) => {
    //protocol.sessionCurrent().setGasBudget(500000000); // increase gas budget
    // console.log(param)
    const passport = new Passport(protocol.sessionCurrent(), param);
    passport.freeze();
};
export const test_guard_launch_number = async (protocol, param) => {
    let permission_id1 = param.get('permission::Permission')[0];
    let maker = new GuardMaker();
...
export const test_guard_launch_permission_builder = async (protocol, param) => {
...
export const test_constant_launch_creator_equal = async (protocol, param) => {
    let permission_id1 = param.get('permission::Permission')[0];
    let permission_id2 = param.get('permission::Permission')[1];
    let maker = new GuardMaker();
